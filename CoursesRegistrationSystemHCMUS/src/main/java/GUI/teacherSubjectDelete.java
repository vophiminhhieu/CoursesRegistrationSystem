...
public class teacherSubjectDelete extends JPanel {
	...
	private Icon deleteIcon = new ImageIcon("image/Menu/Teacher/Account/removesmall.png");
	...
	private void setAll(boolean set) {
			nameLabel.setVisible(set);	creditsLabel.setVisible(set);	deleteBtn.setVisible(set);
	}
	private void prepareGUI() {
		...
		idPanel.setBounds(180,30,180,20);	idPanel.add(id);		idLabel.setBounds(30,30,150,20);
			nameLabel.setBounds(30,60,230,20);	creditsLabel.setBounds(30,90,230,20);
		error.setBounds(30,200,200,20);		setAll(false);	deleteBtn.setBounds(295,200,67,21);
	}
	...
	private ActionListener actionChangeDelete = new ActionListener() {
		public void actionPerformed(ActionEvent e) {
			setAll(false);	error.setText("Success!!");
			subDao.deleteSubject(sub.getId());
		}
	};
	public teacherSubjectDelete(ActionListener obj) {
		setLayout(null);	add(backBtn);	prepareGUI();	backBtn.addActionListener(obj);	setBounds(0, 10, 495, 281);
		add(idPanel);	add(idLabel);		add(nameLabel);		add(creditsLabel);	add(editBtn);	add(error);
		editBtn.addActionListener(actionChangeEdit);	add(deleteBtn);	deleteBtn.addActionListener(actionChangeDelete);
	}
}
